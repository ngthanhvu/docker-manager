    ...
    <div class="volume-list-view">
            ...
            <table class="docker-table">
                <thead>
                    <tr>
                        ...
                        <th class="name-cell"><button class="sort-header" type="button" @click="toggleSort('name')">{{ t('volumesView.name') }}<span class="sort-indicator">{{ getSortIndicator('name') }}</span></button></th>
                        ...
                        <th class="time-cell"><button class="sort-header" type="button" @click="toggleSort('created')">{{ t('volumesView.created') }}<span class="sort-indicator">{{ getSortIndicator('created') }}</span></button></th>
                ...
                <tbody>
                    <tr v-for="vol in paginatedVolumes" :key="vol.Name">
                        ...
                        <td class="time-cell">{{ vol.CreatedAt ? dayjs(vol.CreatedAt).format('YYYY-MM-DD HH:mm') : t('common.notAvailable') }}</td>
        ...
        <div v-if="volumes.length > 0" class="pagination glass-panel">
            ...
            <div class="pager-actions">
                <button class="btn btn-ghost btn-icon" :disabled="currentPage === 1 || bulkDeleting || !!deletingName || pruning" :aria-label="t('common.prev')" :title="t('common.prev')" @click="currentPage--">
                    <ChevronLeft :size="16" />
                </button>
                <span class="pager-page">{{ t('common.page') }} {{ currentPage }} / {{ totalPages }}</span>
                <button class="btn btn-ghost btn-icon" :disabled="currentPage >= totalPages || bulkDeleting || !!deletingName || pruning" :aria-label="t('common.next')" :title="t('common.next')" @click="currentPage++">
